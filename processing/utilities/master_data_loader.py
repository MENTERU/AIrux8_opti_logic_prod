...
def get_comfort_range(master_data: dict, zone: str, month: int) -> tuple[float, float]:
    ...
    try:
        ...
        master_dir = get_data_path("master_data_path")
        excel_path = os.path.join(master_dir, f"MASTER_{master_data['store_info']['name']}.xlsx")
        ...
        if len(zone_month_data) == 0:
            print(f"[get_comfort_range] No data found for zone {zone}, month {month_japanese}")
            return (22.0, 25.0)  # Default comfort range
        
        row = zone_month_data.iloc[0]
        min_temp = float(row["目標室内温度下限"]) if pd.notna(row["目標室内温度下限"]) else 22.0
        max_temp = float(row["目標室内温度上限"]) if pd.notna(row["目標室内温度上限"]) else 25.0
        
        print(f"[get_comfort_range] Zone {zone}, Month {month_japanese}: comfort range {min_temp}-{max_temp}°C")
