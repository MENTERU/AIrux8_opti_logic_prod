...
def plot_area_graph(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            name=power_name,
            line=dict(color=color_mapping.get("power", "blue")),
        ),
        row=1,
        col=col,
        secondary_y=False,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            name=temp_name,
            line=dict(color=color_mapping.get("indoor_temp", "red")),
        ),
        row=1,
        col=col,
        secondary_y=True,
    ...
    for col_name, name, mapping in zip(
        ...
        if col_name in df.columns:
            ...
            if mapping:
                ...
                customdata = df[col_name].map(mapping).fillna(df[col_name].astype(str))
                hovertemplate = (
    ...
    fig.add_trace(
        go.Scatter(
            ...
            showlegend=(col == 1),
            line=dict(color=color_mapping.get("outdoor_temp", "black")),
        ),
        row=1,
        col=col,
        secondary_y=True,
    ...
    fig.update_yaxes(title_text="Power", row=1, col=col, secondary_y=False)
    fig.update_yaxes(title_text="Temperature", row=1, col=col, secondary_y=True)

    # Apply unified ranges if provided
    if power_range is not None:
        fig.update_yaxes(range=power_range, row=1, col=col, secondary_y=False)
    if temp_range is not None:
        fig.update_yaxes(range=temp_range, row=1, col=col, secondary_y=True)
...
def plot_historical_vs_forecast(
    ...
    # Define consistent color mapping for all data series
    color_mapping = {
        "power": "blue",
        "indoor_temp": "red",
        "outdoor_temp": "black",
    ...
    # --- Generate dynamic output filename based on forecast dates ---
    forecast_start_str = forecast_start.strftime("%Y%m%d")
    forecast_end_str = forecast_end.strftime("%Y%m%d")

    # Extract directory and base filename from original output_file
    import os
    ...
    # Create new filename with dates
    dynamic_output_file = os.path.join(
        output_dir,
        f"{name_without_ext}_{forecast_start_str}_{forecast_end_str}{extension}",
    )

    # --- Generate plots ---
    with open(dynamic_output_file, "w") as f:
        ...
        for area in all_areas:
            # Create side-by-side subplot for historical and forecast
            fig_combined = make_subplots(
                ...
                subplot_titles=[
                    ...
                    f"{area} - 予測 ({forecast_start} to {forecast_end})",
                ],
            )

            # Plot historical data (left column)
            plot_area_graph(
                fig_combined,
                hist_pivot,
                area,
            ...
            # Plot forecast data (right column)
            plot_area_graph(
                fig_combined,
                fore_df,
                area,
            ...
            # Update layout with white background and increased width
            fig_combined.update_layout(
                height=400,
                width=1500,  # Slightly increased from 1400
                plot_bgcolor="white",
                paper_bgcolor="white",
                showlegend=True,
            )

            # Add grid styling for better visibility on white background
            fig_combined.update_xaxes(showgrid=True, gridwidth=1, gridcolor="lightgray")
            fig_combined.update_yaxes(showgrid=True, gridwidth=1, gridcolor="lightgray")

            # Write the combined figure
            if area == all_areas[0]:
                f.write(fig_combined.to_html(full_html=False, include_plotlyjs="cdn"))
            else:
                f.write(fig_combined.to_html(full_html=False, include_plotlyjs=False))
        ...
        bar_fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor="lightgray")
    ...
    print(f"Generated plot: {dynamic_output_file}")
...
if __name__ == "__main__":
    ...
    forecast_data_file = "/Users/hussain/Menteru-Github/AIrux8_opti_logic/data/04_PlanningData/Clea/control_type_schedule_20250929.csv"
    mapping_file = (
