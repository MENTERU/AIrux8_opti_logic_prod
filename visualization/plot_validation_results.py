...
def plot_validation_results(plot_validation_only=True, validation_start_dict=None):
    ...
    output_html_file = "/Users/hussain/Menteru-Github/AIrux8_opti_logic/validation_plots.html"
    ...
    with open(output_html_file, 'w') as f:
        ...
        for i, file_name in enumerate(sorted(csv_files)):
            ...
            fig.add_trace(go.Scatter(x=df["Datetime"], y=df["Indoor Temp."], name="Indoor Temp (True)", showlegend=True), row=1, col=1)
            fig.add_trace(go.Scatter(x=df["Datetime"], y=df[temp_pred_col], name="Indoor Temp (Pred)", showlegend=True), row=1, col=1)
            fig.add_trace(go.Scatter(x=df["Datetime"], y=df["A/C Set Temperature"], name="Set Temp", showlegend=True), row=1, col=1)
            fig.add_trace(go.Scatter(x=df["Datetime"], y=df["Outdoor Temp."], name="Outdoor Temp", showlegend=True), row=1, col=1)

            # Add Power Plot
            fig.add_trace(go.Scatter(x=df["Datetime"], y=df["adjusted_power"], name="Power (True)", showlegend=True), row=2, col=1)
            fig.add_trace(go.Scatter(x=df["Datetime"], y=df[power_pred_col], name="Power (Pred)", showlegend=True), row=2, col=1)
